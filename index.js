...
const { parse, getDay, format, set, isWithinInterval, isToday } = require('date-fns');
	...
	getHumanReadable() {
		...
		for (let i = 0; i < weekdays.length; ++i) {
			...
			if (filtered.length == 0) {
				readable.push([`${weekdays[i]}${todayWeekday == i ? ' (hoje)' : ''}`, 'Fechado']);
				continue;
			}

			const sorted = filtered.sort(
				(a, b) => parse(a.open.time, 'HHmm', new Date()) - parse(b.open.time, 'HHmm', new Date()),
			);

			let hr = '';
			for (let j = 0; j < sorted.length; ++j) {
				const start = format(parse(sorted[j].open.time, 'HHmm', new Date()), 'HH:mm');
				const end = format(parse(sorted[j].close.time, 'HHmm', new Date()), 'HH:mm');
				if (start == end) {
					hr += `Aberto 24 horas`;
					break;
				}
			...
			readable.push([`${weekdays[i]}${todayWeekday == i ? ' (hoje)' : ''}`, hr]);
		}
		return readable;
	}
	...
	getDayHours(weekday) {
		...
		for (let i = 0; i < filtered.length; i++) {
			...
			if (f.open.day == weekday) {
				start = set(setDay(new Date(), f.open.day), {
				...
				if (f.close.day != weekday) {
					end = set(setDay(new Date(), f.close.day), { hours: 23, minutes: 59, seconds: 59 });
				} else {
					end = set(setDay(new Date(), weekday), {
				...
				start = set(setDay(new Date(), weekday), { hours: 0, minutes: 0, seconds: 0 });

				end = set(setDay(new Date(), f.close.day), {
					hours: f.close.time.substr(0, 2),
					minutes: f.close.time.substr(2, 4),
					seconds: 0,
					milliseconds: 0,
